...
- Meshes are authored as triangle strips, never lists: every strip repeats its first point, the odd triangles read the other way, and the pass draws with `frontFace(CW)`. On upload, `mesh.getTriangles()` turns the strip into an indexed triangle list of its real triangles, so the joins cost nothing on the GPU and `bufferCount` counts indices. A `dynamicDraw` mesh keeps that layout, so a dirty upload only rewrites the vertex values. A builder that knows which strip entries are one vertex sets `mesh.vertexKeys`, and the upload skips the search; `buildGrid` does for a smooth grid. Back faces are culled unless `mesh.doubleSided` is set, which the open builders (`buildGrid`, `buildRibbon`, an uncapped lathe) do themselves
