function gameInit()
{
    ...
    // outer vertical layout: title on top, grid below (transparent - just for positioning)
    const menu = new UILayout(mainCanvasSize.scale(.5), 1, 20, 30, true);
    menu.addChild(new UIText(vec2(), vec2(400, 60), 'Level Select'));

    // inner 3x2 grid of level select buttons
    // build it fully before adding to outer so outer reads its final size
