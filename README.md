...
**The canvas clear in `updateCanvas` has 36 bytes of safety net you can cut.** The engine only assigns `mainCanvas.width` when the size actually changes, because assigning it reallocates the canvas and makes the browser rebuild the page, which was a real bottleneck on some machines. Assigning it also cleared the canvas and reset the 2D context, so the unchanged path now does that by hand: `clearRect` on both canvases costs 25 bytes, and restoring the transform and blend mode costs 36 more. The engine never needs that second part, it pairs every `save` with a `restore` and every `setAdditiveBlendMode` with its reset. It is there for **your** code: if you call `setAdditiveBlendMode(true)` or apply a transform and never set it back, those lines are what keep the next frame looking right. If you know you always pair yours, delete the `setTransform` and `globalCompositeOperation` lines and take the 36 bytes.
