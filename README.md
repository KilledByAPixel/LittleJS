...
Main was reviewed again through **1.19.4** (September 2026) and the fixes that were free were taken: the `speak` volume overflow, decoded sound samples used without a copy, and the paused update rate. One that was not free was taken anyway, for 25 bytes: only resizing the canvas when its size changes, since resizing every frame was a real bottleneck on some machines. Two allocation savings from main after that release (commits `312895d5` and `c71c25fd`) were taken too, for 7 bytes together: `localPos` only exists on a child, and an object only copies its old position when collision response will read it. That second one deliberately differs from main. Main only checks mass, which is enough there because its particles are separate lightweight objects, but particles are `EngineObject`s here with mass for gravity, so a mass-only check would still copy for every particle every frame. Checking the collision flags as well is what actually spares them, and `npm test` fails if that regresses. That commit's new `attach()` was not taken. Deliberately not taken, because each adds bytes to always-running code for performance or features this branch does not have: the 3D plugins and their collision work, `Shader`, `SpriteAnimation`, `AudioEffect` and the master effect chain, `SoundInstance` playback controls, shared audio buffers, device pixel ratio support, and skipping renders when nothing changed. The API is still aligned to 1.18.25, so the migration notes above stand.
