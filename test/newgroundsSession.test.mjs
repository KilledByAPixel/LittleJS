...
import { NewgroundsPlugin, NewgroundsMedal, medalsInit, medalsReset } from '../dist/littlejs.esm.js';

// A logged in session: the plugin reads the session id from the page url, and every call is a fetch.
// The fetch is stubbed per component, and the keep alive interval is captured instead of scheduled
// so the test can fire it, and so the process can exit.
...
globalThis.fetch = async (url, options) =>
{
    ...
    return { text: async ()=> JSON.stringify({ success: true, result: { component: call.component, data: reply } }) };
};
let keepAlive;
...
test('when logged in the server holds the medals and the local save is left alone', async () =>
{
    ...
    replies['ScoreBoard.getBoards'] = { scoreboards: [] };
    ...
    // an unlock only lands once the server confirms it
    replies['Medal.unlock'] = { medal: { id: 1, unlocked: true }, medal_score: 5 };
    m1.unlock();
    ...
    replies['Medal.unlock'] = { medal: { id: 3, unlocked: true }, medal_score: 5 };
    keepAlive();
    await flush();
    ...
    // a server refusal stays pending too
    replies['Medal.unlock'] = { success: false, error: { message: 'no such medal' } };
