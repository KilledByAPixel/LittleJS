...
globalThis.fetch = async (url, options) =>
{
    ...
    return { text: async ()=> JSON.stringify({ success: true, result: { component: input.execute.component, data } }) };
};
...
test('getScores asks for the whole board without the session, and for a user or social with it', async () =>
{
    ...
    assert.deepEqual(board.execute.parameters, { id: 3, social: false, skip: 0, limit: 10, period: 'A' });
    ...
    assert.equal(inputs.find(i => i.execute.component == 'Medal.getList').session_id, 'abc123', 'other calls keep the session');
});
